...
public class PagosModel {
	...
	/*
	// En esta consulta tenemos las actividades ya pagadas
	public static final String get_actividad = "SELECT * FROM pagos p JOIN reservas r ON p.id_reservas=r.id_reserva JOIN actividades a ON r.actividad = a.id_actividad WHERE p.dni_clientes=? AND a.nombre=?";

	public String getReservaActividad(String dni, String nombre_actividad) {
		List<Object[]> l = bd.executeQueryArray(get_reserva, dni, nombre_actividad);
		if (l.isEmpty()) {
			return null;
		}
		return l.get(0)[0].toString();
