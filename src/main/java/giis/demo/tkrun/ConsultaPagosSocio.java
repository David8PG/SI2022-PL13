...
public void RellenarTabla(JTable tabla, String Inicio, String Fin) {
	...
	for(int k=0;k<i;k++) {
		...
		Date fe=null;
		try {
			fe = (Date) sdf.parse(mRes[k][3].toString());
			orden[k]=fe;
		...
		//Estado	
		//pagado=pagosmodel.getPagoReserva(mRes[k][0].toString()); estae
		if(pagado) {
	...
	for(int k=i;k<y+i;k++) {
		//Precio 
		datosTabla[k][0]=actividadesmodel.getPrecioSocio(mIns[k-i][2].toString())+" €";
		//Fecha
		Date fe=null;
		try {
			fe = (Date) sdf.parse(mIns[k-i][3].toString());
			orden[k]=fe;
		} catch (ParseException e) {
		}
		String f=sdf.format(fe);
